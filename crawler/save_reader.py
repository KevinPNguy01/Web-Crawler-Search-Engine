...
with shelve.open("frontier_backup.shelve") as save, open("links.txt", "w") as file:
    links = sorted([item[0] for item in save.values()])
    for link in links:
        file.write(f"{link}\n")

with shelve.open("frequencies_backup.shelve") as save, open("frequencies.txt", "w") as file:
